...
class PollView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        is_alumno = True if Curso.objects.filter(id=id_curso, alumno=request.user.id) else False
        poll_done = True if Encuesta.objects.filter(curso_id= id_curso, alumno_id = request.user.id) else False
...
class DetailView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        time_to_poll = True if str(curso.dia_final) == timezone.now().strftime('%Y-%m-%d') else False
